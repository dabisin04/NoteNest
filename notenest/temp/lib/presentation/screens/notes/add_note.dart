...
class _AddOrEditNoteScreenState extends State<AddOrEditNoteScreen> {
  ...
  void _submit(User user) {
    ...
    if (!isEditing && _selectedFiles.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
                ...
                children: [
                  ...
                  BlocBuilder<NoteBloc, NoteState>(
                    builder: (context, state) {
                      ...
                      } else if (state is NoteFilesLoaded) {
                        return Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: state.files.map((file) {
                            return Text(
                                file['fileUrl'] ?? 'Archivo sin nombre');
                          }).toList(),
                        );
                      } else {
                        return Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children:
                              _selectedFiles.map((f) => Text(f.name)).toList(),
                        );
                      }
                    },
                  ),
                  if (_existingFiles.isNotEmpty) ...[
                    const SizedBox(height: 12),
                    const Text('Archivos ya subidos:'),
                    ..._existingFiles.map((f) => ListTile(
                          title: Text(f['fileUrl']),
                          trailing: IconButton(
